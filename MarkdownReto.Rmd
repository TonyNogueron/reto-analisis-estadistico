...
```{r}
m1 = lm(Y~A+B+C+D+E+F+G+H, dataset=tipo1)
summary(m1)
step(object = m1, direction = "both", trace = 1)
m1 = lm(formula = Y ~ B + C + D + E + F, dataset = tipo1)
summary(m1)
```
...
```{r}
m2 = lm(Y~A+B+C+D+E+F+G+H, dataset=tipo2)
summary(m2)
step(object = m2, direction = "both", trace = 1)
m2 = lm(formula = Y ~ B + C + D + E + F, dataset = tipo2)
summary(m2)
```

### Modelo para el tipo de material 3
``` {r}
m3 = lm(Y~A+B+C+D+E+F+G+H, dataset=tipo3)
summary(m3)
step(object = m3, direction = "both", trace = 1)
m3 = lm(formula = Y ~ B + C + D + E + F, dataset = tipo3)
summary(m3)
